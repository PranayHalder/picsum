 ...
 const editImage = (event, image, done) => {
   ...
   const position = async (e) => {
    ...
    await navigator.geolocation.getCurrentPosition(
      position => {
        ...
        axios.post('https://us1.locationiq.com/v1/reverse?key=pk.66820c46f6ad77c1a22602c6210ac5cc&lat='+position.coords.latitude+'&lon='+position.coords.longitude+'&format=json').then((result)=>{
          setFetchloc(false);
   ...
   const handleSuggesstion = (e) => {
    let value = e.target.value;
     ...
     {
      setSgstate(true);
      axios.post('https://us1.locationiq.com/v1/search?key=pk.66820c46f6ad77c1a22602c6210ac5cc&q={'+value+'}&format=json').then((result)=>{
   ...
   const getLatLong = data => () => {
    setSgstate(false);
    axios.post('https://us1.locationiq.com/v1/reverse?key=pk.66820c46f6ad77c1a22602c6210ac5cc&lat='+data.lat+'&lon='+data.lon+'&format=json').then((result)=>{ 
  ...
  const handleTextarea = (e) => {
   ...
   const config = {
     ...
     axios.post('http://localhost:8888/api/upload.php',formData,config).then((result)=>{
      if(result.data.message == 'success'){
        MySwal.fire({
          icon: 'success',
          title: 'Yay',
